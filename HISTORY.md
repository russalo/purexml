...
|---|---|---|---|---|---|
| 0.14.1 | n/a | 2026-06-28 | **Patch — license: MIT.** Adds a `LICENSE` (MIT, © 2026 Russell Pfister), sets `license = {text = "MIT"}` + the OSI MIT classifier in `pyproject.toml` (table form keeps the classifier without the PEP 639 SPDX-string warning on the setuptools≥65 floor), and replaces the "license deferred" placeholders in README/STACK with MIT. Rationale (Russell): give-it-away, zero-dep infrastructure → pure open source, maximum reuse, no commercial-upside concern (the deliberate opposite of file-observer's AGPL-3.0 + dual-commercial); MIT is one-way compatible with AGPL, so purexml stays clean as a future file-observer dependency. Sets the **pure\* family default** (purecfb, puresniff) unless a lib flags a reason to differ. PyPI publish + name claim stay deferred (license settled; publish trigger separate). No code/parse change; LOGIC unchanged, SCHEMA n/a. _(HISTORY only, no RFC — part of v0.14.)_ | _(no RFC — patch)_ | _(part of v0.14)_ |
