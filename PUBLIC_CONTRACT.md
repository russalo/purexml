...
> **⚠️ NOT YET BINDING — purexml is pre-1.0 (currently v0.10.0).** No public
...
> `defusedxml` at its defaults — **plus, as of v0.10, `purexml.minidom`**
> (`parse`/`parseString` → stdlib `Document`) and `purexml.common` (the
> `DefusedXmlException` catch-site alias); `sax` is the next breadth module
> (see `docs/ROADMAP-to-1.0.md`) — plus **opt-in, default-off** additions (`Limits`
